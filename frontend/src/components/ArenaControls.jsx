...
// Operator control for the arena: set the API key + timer, start a deal, and
// reveal — without leaving the big-screen view. Rendered as a right-docked
// drawer over a scrim so it never overlaps the brand or the offers. The trigger
// (⚙️) lives in the arena header; `open`/`onClose` are controlled by Arena.
export default function ArenaControls({ listingId, open, onClose }) {
  ...
  return (
    <>
      ...
      <aside className="op-drawer" role="dialog" aria-label={t("opPanel")}>
        <div className="op-head">
          <div>
            <div className="op-title">🎛️ {t("opPanel")}</div>
            <div className="op-subtitle">{t("opSubtitle")}</div>
          </div>
          <button className="op-x" onClick={onClose} aria-label="close">
            ✕
          </button>
        </div>
